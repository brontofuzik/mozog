...
{
    public class DepthLimitedMinimaxSearch : IAdversarialSearch
    {
        ...
        public DepthLimitedMinimaxSearch(IGame game, bool prune = true, bool tt = true)
        {
            this.game = game;
            this.prune = prune;
        ...
        public (IAction move, double eval) MakeDecision(IState state, int depth = Int32.MaxValue)
        {
            Metrics.Set(NodesExpanded_Move, 0);
            transTable?.Clear();
            ...
            var result = Minimax(state, depth, prune, alpha, beta);

            return (result.action, result.utility);
        }

        public (IAction move, double eval, int nodes) MakeDecision_DEBUG(IState state, int depth = Int32.MaxValue)
        {
            var result = MakeDecision(state, depth);
            return (result.move, result.eval, Metrics.Get<int>(NodesExpanded_Move));
        }

        private (double utility, IAction action) Minimax(IState state, int depth, bool prune, double alpha, double beta)
        {
            var alphaOrig = alpha;
            var betaOrig = beta;

            // Transposition table
            if (transTable != null && TranspositionLookup(state, depth, ref alpha, ref beta, out var cached))
                return cached;
            ...
            if (depth == 0 || game.IsTerminal(state))
                return (game.EvaluateState(state), null);
            ...
            {
                double utility = Minimax(newState, depth - 1, prune, alpha, beta).utility;
            ...
            if (transTable != null)
                TranspositionStore(state, depth, bestEval, alphaOrig, betaOrig, bestAction);
        ...
        private bool TranspositionLookup(IState state, int depth, ref double alpha, ref double beta, out (double eval, IAction move) cached)
        {
        ...
        private void TranspositionStore(IState state, int depth, double bestEval, double alphaOrig, double betaOrig, IAction bestAction)
        {
            TTFlag flag;
            ...
            var entry = new TTEntry { Depth = depth, Eval = bestEval, Action = bestAction, Flag = flag };
